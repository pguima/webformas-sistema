<div class="space-y-6">
    ...
    <x-ds::card title="SEÇÃO 4 — Performance" description="PageSpeed Insights (Google).">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        @if($pageSpeed)
            ...
            <div class="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-2">
                @foreach($strategies as $strategyKey => $strategyLabel)
                    ...
                    <x-ds::card :title="'PageSpeed — ' . $strategyLabel" :description="'Resultados para ' . $strategyLabel . '.'">
                        @if($strategyError)
                            <x-ds::alert variant="danger" icon="solar:danger-circle-linear">
                                {{ $strategyError }}
                            </x-ds::alert>
                        ...
                        @if($strategyData)
                            ...
                            <div class="mt-4 overflow-x-auto">
                                <table class="w-full text-left text-sm">
                                    <thead class="border-b border-(--border-subtle) bg-(--surface-hover) text-xs font-medium uppercase tracking-wider text-(--text-secondary)">
                                        <tr>
                                            <th class="px-4 py-3 font-semibold">LCP</th>
                                            <th class="px-4 py-3 font-semibold">CLS</th>
                                            <th class="px-4 py-3 font-semibold">TTFB</th>
                                            <th class="px-4 py-3 font-semibold">Speed Index</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr class="border-b border-(--border-subtle)">
                                            <td class="px-4 py-3 text-(--text-secondary)">{{ isset($strategyData['metrics']['lcp_ms']) && $strategyData['metrics']['lcp_ms'] !== null ? round($strategyData['metrics']['lcp_ms'] / 1000, 2) . 's' : __('app.common.dash') }}</td>
                                            <td class="px-4 py-3 text-(--text-secondary)">{{ $strategyData['metrics']['cls'] ?? __('app.common.dash') }}</td>
                                            <td class="px-4 py-3 text-(--text-secondary)">{{ isset($strategyData['metrics']['ttfb_ms']) && $strategyData['metrics']['ttfb_ms'] !== null ? round($strategyData['metrics']['ttfb_ms'] / 1000, 2) . 's' : __('app.common.dash') }}</td>
                                            <td class="px-4 py-3 text-(--text-secondary)">{{ isset($strategyData['metrics']['speed_index_ms']) && $strategyData['metrics']['speed_index_ms'] !== null ? round($strategyData['metrics']['speed_index_ms'] / 1000, 2) . 's' : __('app.common.dash') }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
        ...
        @else
            <div class="mt-4 text-sm text-(--text-secondary)">
                Clique em <span class="font-medium">Analisar PageSpeed</span> para gerar os indicadores.
            </div>
        @endif
