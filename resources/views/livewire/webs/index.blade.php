<div class="space-y-6">
    ...
    <x-ds::card class="mt-6">
        ...
        @else
            <x-ds::table :headers="[__('app.webs.table.name'), __('app.webs.table.client'), __('app.webs.table.url'), __('app.webs.table.status'), __('app.webs.table.actions')]">
                @forelse($webs as $web)
                    <tr class="border-b border-(--border-subtle) transition-colors hover:bg-(--surface-hover)" wire:key="{{ $web->id }}">
                        ...
                        <x-ds::table-cell>
                            <div class="text-sm text-(--text-secondary)">{{ $web->client?->name }}</div>
                        </x-ds::table-cell>
                        <x-ds::table-cell>
